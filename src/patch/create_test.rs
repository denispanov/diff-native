...
fn test_context_zero_behavior() {
    let prev_context_block = ["context1".to_string(), "context2".to_string()];
    let context = 0;
...
fn test_line_counting_logic() {
    let mut old_line = 1;
    let mut new_line = 1;
    let added_lines = ["added1".to_string(), "added2".to_string()];
    new_line += added_lines.len();

    assert_eq!(old_line, 1);
    assert_eq!(new_line, 3);
    let removed_lines = ["removed1".to_string()];
    old_line += removed_lines.len();

    assert_eq!(old_line, 2);
    assert_eq!(new_line, 3);
    let unchanged_lines = ["unchanged1".to_string(), "unchanged2".to_string()];
